...
def main(args, verbose=True):
    ...
    # different
    outfile_factory = functools.partial(
        OutFileFormat,
        args.output,
        "w",
...
def write(outfile, data):
    ...
    if isinstance(outfile, md.formats.XTCTrajectoryFile):
        ...
        outfile.write(
            ...
            data.get("cell_lengths", None),
            data.get("cell_angles", None),
            data.get("velocities", None),
            data.get("kineticEnergy", None),
            data.get("potentialEnergy", None),
...
def read(infile, chunk, stride, atom_indices):
    ...
    if isinstance(infile, md.formats.PDBTrajectoryFile):
        ...
        data = {
            "xyz": infile.positions[::stride, atom_indices, :],
            "topology": topology,
        }
        if infile.unitcell_lengths is not None:
            data["cell_lengths"] = np.array(
                [infile.unitcell_lengths] * len(data["xyz"]),
            )
    ...
    if "cell_lengths" in out_fields:
        if "cell_lengths" in data:
            ...
            (
                ...
                beta,
                gamma,
            ) = md.utils.unitcell.box_vectors_to_lengths_and_angles(
                data["box"][:, 0],
                data["box"][:, 1],
                data["box"][:, 2],
            )
