...
def traj(tmpdir):
    ...
    # Changed from h5 to xtc -> pytables requires Numpy < 2.0
    fn = f"{tmpdir}/ref.xtc"
    traj.save(fn)
...
def test_index(traj):
    ...
    print(os.getcwd())
    subprocess.check_call(["python", "mdconvert.py", in_fn, "-i", "4", "-o", out_fn])
    frame4 = md.load(out_fn)
    assert np.allclose(frame4.xyz, traj[4].xyz)
...
def test_slice(traj):
    ...
    subprocess.check_call(
        ["python", "./mdconvert.py", in_fn, "-i", "1:5:2", "-o", out_fn]
    )
    frame13 = md.load(out_fn)
    assert np.allclose(frame13.xyz, traj[1:5:2].xyz)
...
def test_pairwise(traj, extension, monkeypatch):
    ...
    def test_base(traj, extension, monkeypatch):
        ...
        for ext2 in extensions:
            ...
            command1 = ["python", "mdconvert.py", in_fn, "-o", out_fn, "-c 6"]
            ...
            if ext1 in ["lh5"] or ext2 in ["lh5"]:
                ...
                decimal = 6
            assert np.allclose(out1.xyz, traj.xyz, decimal=decimal)
            assert np.allclose(out2.xyz, traj.xyz[:, atom_indices], decimal=decimal)
            assert np.allclose(out3.xyz, traj.xyz[::3], decimal=decimal)
    
            if ext1 not in ["lh5"] and ext2 not in ["lh5"]:
                # binpos doesn't save unitcell information
                assert np.allclose(out1.unitcell_vectors, traj.unitcell_vectors, decimal=2)
                assert np.allclose(out2.unitcell_vectors, traj.unitcell_vectors, decimal=2)
                assert np.allclose(out3.unitcell_vectors, traj.unitcell_vectors[::3], decimal=2)
            ...
            if all(e in ["xtc", "trr", "nc", "h5"] for e in [ext1, ext2]):
                ...
                if all(e in ["nc"] for e in [ext1, ext2]):
                    with monkeypatch.context() as m:
                        ...
                        assert np.allclose(out1.time, traj.time)
                        assert np.allclose(out2.time, traj.time)
                        assert np.allclose(out3.time, traj.time[::3])
            ...
            if ext2 in ["pdb", "h5", "lh5"]:
                ...
                # read from disk
                assert np.allclose(out1.topology, traj.topology)
                assert np.allclose(out2.topology, traj.topology.subset(atom_indices))
                assert np.allclose(out3.topology, traj.topology)
